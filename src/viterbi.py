...
def viterbi(
    ...
    # Mapping from state to index and back
...
def generate_viterbi_output(test_data, transition_probs, emission_probs):
    ...
    with open("../EN/dev.p2.out", "w") as f:
        for sentence in test_data:
            ...
            mapped_sentence = [
                word if word in known_words else "#UNK#" for word in sentence
            ]

            tag_sequence = viterbi(transition_probs, emission_probs, mapped_sentence)

            # Output tags for the original words (but sequence aligns to mapped_sentence)
